...
for file in files:
    ...
    output_data = pd.DataFrame(index=range(len(unique_plate_names) * 2 + 2), columns=['Concentration'] + list(T['Drug Name'].unique()))
    ...
    for j, plate_name in enumerate(unique_plate_names):
        ...
        output_data.loc[row_start + 1:, 'Concentration'] = np.repeat(unique_concentrations, len(drug_rows))[:len(output_data)-row_start-1]
